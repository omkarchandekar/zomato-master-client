...
function LargeNav({SignUp, SignIn}){
    ...
    return(
        <>
          <div  className='hidden lg:block container px-20 mx-auto'>
            <div className='gap-4 w-full items-center justify-around flex'>
              ...
              <div className='w-3/4 bg-white shadow-md p-3 flex items-center gap-3 border-gray-200 rounded'>
              ...
              {reduxState?.fullName ? (
                <div className='relative w-20'>
                  <div onClick={() => setIsDropDownOpen((prev) => !prev)} className='border border-gray-200 text-zomato-400 w-full h-20 rounded-full'>
                    <img src="https://cdn1.vectorstock.com/i/1000x1000/36/15/businessman-character-avatar-isolated-vector-12613615.jpg" alt="" className="w-full h-full rounded-full object-cover" /> 
                  </div>
                  {isDropDownOpen && (
                  <div className="absolute shadow-lg py-3 -bottom-20 -right-4 w-full bg-white z-20 flex flex-col gap-2">
                ...
                <div className='flex gap-4'>
                  <button className='text-gray-500 text-xl hover:text-gray-800' onClick={SignIn}>Login</button>
                  <button className='text-gray-500 text-xl hover:text-gray-800' onClick={SignUp}>Sign Up</button>
                </div>
              )}
            </div>  
          </div>   
